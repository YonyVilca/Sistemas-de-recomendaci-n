...
movies = pd.read_csv('/workspaces/Paralela/Mineria/Coseno/movies.csv')

# Reemplazar calificaciones de 0 con NaN para indicar ítems no calificados
ratings['rating'] = ratings['rating'].replace(0, np.nan)

# Crear una matriz usuario-ítem
...
# Elegir un usuario y película para la predicción
user_id = 6  # Ejemplo de ID de usuario
movie_id = 108932  # Ejemplo de ID de película
...
# Mostrar la predicción denormalizada con más decimales y el nombre de la película
...
ratings_file_size = os.path.getsize('/workspaces/Paralela/Mineria/Coseno/ratings.csv') / (1024 ** 2)  # Tamaño en MB
movies_file_size = os.path.getsize('/workspaces/Paralela/Mineria/Coseno/movies.csv') / (1024 ** 2)  # Tamaño en MB
