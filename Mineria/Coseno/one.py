...
# Cargar el dataset de MovieLens Small
file_path = '/workspaces/Paralela/Mineria/Coseno/ratings.csv'
ratings_df = pd.read_csv(file_path)

# Cargar el archivo de nombres de películas
movies_file_path = '/workspaces/Paralela/Mineria/Coseno/movies.csv'
...
# Convertir la matriz de ratings de pandas a numpy
@jit(nopython=True)
def compute_deviations_np(ratings):
    ...
    # Iterar sobre cada par de películas y calcular la desviación y frecuencia
    for user_ratings in ratings:
        # Iterar sobre las combinaciones de películas
    ...
    # Calcular promedio de desviación por cada par de ítems
    for item1 in range(num_items):
        for item2 in range(num_items):
...
def slope_one_prediction_np(user_idx, item_idx, ratings, deviations, frequencies):
    ...
    # Iterar sobre todos los ítems que el usuario ha calificado
