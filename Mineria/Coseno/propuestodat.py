...
# Paso 1: Función para cargar el dataset de calificaciones desde un archivo .dat sin cabecera
def load_movielens(file_path):
    """
    Cargar el dataset MovieLens desde un archivo .dat sin cabecera con la estructura: userId::movieId::rating::timestamp.
    
    Parámetros:
    - file_path: Ruta del archivo de calificaciones .dat
    
    Retorna:
    - DataFrame de calificaciones
    """
    # Leer archivo .dat sin cabecera, usando el delimitador '::'
    ratings = pd.read_csv(file_path, sep='::', engine='python', names=['userId', 'movieId', 'rating', 'timestamp'], encoding='latin1')
    
    # Retornar solo las columnas relevantes
    return ratings[['userId', 'movieId', 'rating']]

# Paso 2: Función para cargar los nombres de las películas desde un archivo .dat sin cabecera
def load_movie_titles(movie_file_path):
    """
    Cargar los nombres de las películas desde un archivo .dat sin cabecera con la estructura: movieId::title::genres.
    ...
    """
    # Leer archivo .dat sin cabecera, usando el delimitador '::'
    movies = pd.read_csv(movie_file_path, sep='::', engine='python', names=['movieId', 'title', 'genres'], encoding='latin1')
    
    # Retornar solo movieId y title
    return movies[['movieId', 'title']]

# Paso 3: Cargar los datasets de calificaciones y películas
file_path_ratings = '/workspaces/Paralela/Mineria/Coseno/ratings10m.dat'  # Ruta al archivo .dat de calificaciones
file_path_movies = '/workspaces/Paralela/Mineria/Coseno/movies10m.dat'   # Ruta al archivo .dat de películas
...
movie_id = 15  # ID de la película de ejemplo
